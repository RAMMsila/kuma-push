...
read -p "Введите URL для параметра --url (cсылка из Uptime-Kuma): " URL
read -p "Введите хост для параметра --ping-host (ip/домен до которого будет идти TCP запрос): " PING_HOST
